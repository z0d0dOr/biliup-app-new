    ...
    <div class="publish-time-editor">
            ...
            <div class="publish-time-editor-panel-header">
                ...
                <span class="publish-time-editor-header-meta">
                    {{
                        loading
                            ? '加载中...'
                            : `${runningCards.length} 个视频 / ${seasonCount} 个合集`
                    }}
                </span>
            </div>
            <div v-if="loading || runningCards.length === 0" class="publish-time-editor-empty">
                <el-empty
                    :description="loading ? '正在加载进行中视频...' : '暂无进行中视频'"
                    :image-size="88"
                />
            </div>
            <div
                    ...
                    <div
                        ...
                        :class="{
                            'publish-group-row-header--dragging':
                                draggingSeasonKey === seasonRow.key,
                            'publish-group-row-header--disabled': seasonRow.draggableCount === 0
                        }"
                        @click="toggleSeasonGroup(seasonRow.key)"
                        @mousedown.left.stop="handleSeasonPointerDown($event, seasonRow.key)"
                    >
                        ...
                        <div class="publish-date-row-header-right">
                            ...
                            <el-icon
                                class="publish-group-arrow"
                                :class="{ collapsed: isSeasonGroupCollapsed(seasonRow.key) }"
                            >
                                <ArrowDown />
                            </el-icon>
                        </div>
                    ...
                    >
                        ...
                        >
                            <div class="publish-time-video-card-title-row">
                                <div class="publish-time-video-card-title">
                                    {{ task.videoTitle }}
                                </div>
                                <div
                                    class="publish-time-video-card-season"
                                    :title="task.seasonTitle"
                                >
                                    {{ task.seasonTitle }}
                                </div>
                            </div>
        ...
        <section class="publish-time-editor-panel">
            ...
            <div class="publish-date-list">
                ...
                >
                    <div
                        class="publish-date-row-header publish-group-row-header"
                        @click="toggleDateGroup(dateRow.key)"
                    >
                        ...
                        <div class="publish-date-row-header-right">
                            ...
                            <el-icon
                                class="publish-group-arrow"
                                :class="{ collapsed: isDateGroupCollapsed(dateRow.key) }"
                            >
                                <ArrowDown />
                            </el-icon>
                        </div>
                        ...
                        <div
                            ...
                            :class="{
                                'publish-time-video-card--changed':
                                    task.isChanged && !!task.scheduleDateKey,
                                'publish-time-video-card--selected': isCardSelected(task.id),
                                'publish-time-video-card--dragging': isCardDragging(task.id)
                            }"
                            :data-date-key="dateRow.key"
            ...
            <div class="publish-time-submit-bar">
                <div class="publish-time-submit-meta">
                    ...
                    <span v-if="submitProgress"
                        >提交中 {{ submitProgress.done }}/{{ submitProgress.total }}</span
                    >
                </div>
                <el-button
                    type="primary"
                    :loading="submitting"
                    :disabled="
                        loading || !allVideosScheduled || changedCards.length === 0 || submitting
                    "
                    @click="submitChanges"
                >
                    提交修改
...
type CardDragPayload = { type: 'card'; id: string } | { type: 'cards'; ids: string[] }
...
const rebuildWorkingCards = (videos: any[]) => {
    ...
    workingCards.value = list
        ...
        .map((video: any) => {
            const dtime =
                typeof video?.dtime === 'number' && video.dtime > 0
                    ? Number(video.dtime)
                    : undefined
...
const seasonRows = computed(() => {
    ...
    return Array.from(grouped.entries())
        .map(([key, tasks]) => ({
            key,
            label: key,
            draggableCount: tasks.filter(task => task.isUnscheduled).length,
    ...
    return availableDateList.value.map(item => ({
        ...
        tasks: (grouped.get(item.key) || []).slice().sort((a, b) => {
            ...
            return (
                ...
                aTime - bTime ||
                a.videoTitle.localeCompare(b.videoTitle, 'zh-CN')
            )
        })
    }))
})

const scheduledVideoCount = computed(
    () => runningCards.value.filter(task => task.scheduleDateKey).length
)
const unscheduledRunningVideoCount = computed(
    () => runningCards.value.filter(task => !task.scheduleDateKey).length
)
const changedCards = computed(() => runningCards.value.filter(card => card.isChanged))
const allVideosScheduled = computed(
    () =>
        runningCards.value.length > 0 &&
        runningCards.value.every(card => Boolean(card.scheduleDateKey))
)
...
const getCardDragPayload = (cardId: string): CardDragPayload => {
    const selectedIds = isCardSelected(cardId) ? selectedCardIds.value.slice() : [cardId]
    return selectedIds.length > 1
        ? { type: 'cards', ids: selectedIds }
        : { type: 'card', id: cardId }
}

const getSeasonUnscheduledCardIds = (seasonKey: string) =>
    workingCards.value
        .filter(card => String(card.seasonTitle || '') === seasonKey && !card.assignedDateKey)
        .sort((a, b) => a.orderIndex - b.orderIndex)
        .map(card => card.id)

const getCardDragLabel = (payload: CardDragPayload, seasonKey?: string) => {
    if (seasonKey) {
        const count = payload.type === 'cards' ? payload.ids.length : 1
        return `${seasonKey} (${count})`
    }
...
const assignCardToDate = (cardId: string, dateKey: string) => {
    ...
    if (dateKey && dateKey === latest.dateKey) {
        ...
        !card.assignedDtime ||
        formatDateKey(new Date(card.assignedDtime * 1000)) !== dateKey
    ) {
...
const handleGlobalPointerMove = (event: MouseEvent) => {
    ...
    if (!customCardDrag.value.active) {
        customCardDrag.value = {
            ...
            x: event.clientX + 14,
            y: event.clientY + 14,
            label: getCardDragLabel(pendingCardDrag.payload, pendingCardDrag.seasonKey)
        }
        if (pendingCardDrag.seasonKey) {
            draggingCardIds.value = []
            draggingCardId.value = ''
            draggingSeasonKey.value = pendingCardDrag.seasonKey
        } else {
            draggingCardIds.value =
            ...
            draggingCardId.value =
                pendingCardDrag.payload.type === 'card' ? pendingCardDrag.payload.id : ''
...
const handleGlobalPointerUp = (event: MouseEvent) => {
    ...
    const activeSeasonKey = pendingCardDrag.seasonKey || ''
    ...
    if (activeSeasonKey && target?.type === 'left') return
...
const handleSeasonPointerDown = (event: MouseEvent, seasonKey: string) => {
    if (event.button !== 0) return
    const cardIds = getSeasonUnscheduledCardIds(seasonKey)
    if (cardIds.length === 0) return
    pendingCardDrag = {
        startX: event.clientX,
        startY: event.clientY,
        payload:
            cardIds.length > 1 ? { type: 'cards', ids: cardIds } : { type: 'card', id: cardIds[0] },
        seasonKey
    }
}
...
const getDragPayload = (
    ...
    | { type: 'season'; key: string }
    | null => {
    if (draggingCardIds.value.length > 1)
        return { type: 'cards', ids: draggingCardIds.value.slice() }
...
const submitChanges = async () => {
    ...
    try {
        ...
        const submitIntervalMs = Math.max(
            0,
            Math.round(Number(submitIntervalSeconds.value || 0) * 1000)
        )
        for (let index = 0; index < currentChangedCards.length; index++) {
            ...
            if (card.assignedDateKey) {
                if (card.assignedDateKey === latestSchedulable.dateKey) {
                    ...
                    nextDtime = buildRandomDtimeForDateKey(
                        card.assignedDateKey,
                        globalTimeRange.value || DEFAULT_PUBLISH_TIME_RANGE
                    )
                    if (!nextDtime)
                        throw new Error(`随机时间范围格式无效: ${globalTimeRange.value}`)
                }
            }
        ...
        utilsStore.showMessage(
            `已提交 ${currentChangedCards.length} 个视频的发布时间修改`,
            'success'
        )
    } catch (error) {
        ...
        utilsStore.showMessage(
            `批量提交失败: ${error}，可再次点击继续提交剩余 ${remainingCount} 项`,
            'error'
        )
    } finally {
...
.publish-group-row-header--disabled {
    cursor: pointer;
}
